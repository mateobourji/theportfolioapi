...
class CleaningsRepository(BaseRepository):
    ...
    async def update_cleaning_by_id(self, *, id: int, cleaning_update : CleaningUpdate) -> CleaningInDB:
        ...
        try:
            updated_cleaning = await self.db.fetch_one(
                query=UPDATE_CLEANING_BY_ID, values=cleaning_update_params.dict()
            )
    ...
    async def delete_cleaning_by_id(self, *, id: int) -> CleaningInDB:

        cleaning = await self.db.fetch_one(query=DELETE_CLEANING_BY_ID, values={"id": id})
